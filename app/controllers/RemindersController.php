	...
	public function postRemind()
	{
		...
		if($validator->fails()){
			...
			if($user->count()){
				...
				if($user->save()){
					Mail::send('emails.auth.forgot', array('link' => URL::route('password.recover', $codigo), 'nombre' => $user->nombre, 'password' => $password, 'user' => $user->user), function($message) use ($user){
